...
namespace NCDK.MolViewer
{
        ...
        : BindableBase
    {
        ...
        private bool _ValidSmiles = true;
        ...
        private DelegateCommand _CopyImageCommand;

        private MolWindow molWindow;

        public AppearanceViewModel(MolWindow molWindow)
        {
            this.molWindow = molWindow;
        }

        public bool ValidSmiles
        {
            get => _ValidSmiles;
            set
            {
                var changed = _ValidSmiles == value;
                this.SetProperty(ref this._ValidSmiles, value);
                if (changed)
                    this.RaisePropertyChanged(nameof(ValidSmiles));
            }
        }
        ...
        private static IChemObject ChemObjFromSmiles(string text)
        {
            IChemObject o;
            ...
            else
            {
                ...
                try
                {
                    var romol = RDKit.RWMol.MolFromSmiles(text);
                    if (romol != null)
                    {
                        text = RDKit.RDKFuncs.MolToSmiles(romol);
                    }
                    else
                    { 
                        romol = RDKit.RWMol.MolFromSmarts(text);
                        if (romol != null)
                            text = RDKit.RDKFuncs.MolToSmarts(romol);
                    }
                ...
                if (value != null)
                {
                    switch (value)
                    {
                        case IAtomContainer mol:
                            ...
                            else if (!GeometryUtil.Has2DCoordinates(mol))
                            {
        ...
        public DelegateCommand CopyImageCommand
        {
            get { return _CopyImageCommand = _CopyImageCommand ?? new DelegateCommand(CopyImage); }
        }
        ...
        private void CopyImage()
        {
            ...
            var bmp = molWindow.ObjectBox.depiction.ToBitmap();
    ...
    public class BoolToInfoBrushConverter : IValueConverter
    {
